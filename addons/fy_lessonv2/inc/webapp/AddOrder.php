...
$lesson = pdo_get($this->table_lesson_parent, array('uniacid'=>$uniacid,'id'=>$id));
if (empty($lesson)) {
	$jsonData = array(
		'code' => -1,
		'message' => '课程不存在或已下架',
...
if ($orderid) {
	...
	if($deduct_integral>0){/* 扣除会员积分 */
		load()->model('mc');
		mc_credit_update($uid, 'credit1', '-'.$deduct_integral, array(0, $module_title.'购买课程使用积分抵扣，sn:'.$ordersn));
	}
