...
class Cosapi {
     ...
     */
    public static function createFolder($bucket, $folder, $bizAttr = null, $qcloud) {
        ...
        $url = self::generateResUrl($bucket, $folder, $qcloud);
        $signature = Auth::createReusableSignature($expired, $bucket, $filepath = null, $qcloud);
    ...
    public static function listFolder(
        ...
        return self::listBase($bucket, $folder, $num, $pattern, $order, $context, $qcloud);
    }
    ...
    public static function prefixSearch(
        ...
        return self::listBase($bucket, $prefix, $num, $pattern, $order, $context, $qcloud);
    }
   ...
   /*
     ...
     * @param  string  $folder       目录路径
     */
    public static function statFolder($bucket, $folder, $qcloud) {
        $folder = self::normalizerPath($folder, True);

        return self::statBase($bucket, $folder, $qcloud);
    }
    ...
    private static function listBase(
            $bucket, $path, $num = 20, $pattern = 'eListBoth', $order = 0, $context = null, $qcloud) {
        $path = self::cosUrlEncode($path);
        $expired = time() + self::EXPIRED_SECONDS;
        $url = self::generateResUrl($bucket, $path, $qcloud);
        $signature = Auth::createReusableSignature($expired, $bucket, $filepath = null, $qcloud);
     ...
     */
    private static function statBase($bucket, $path, $qcloud) {
        $path = self::cosUrlEncode($path);
        $expired = time() + self::EXPIRED_SECONDS;
        $url = self::generateResUrl($bucket, $path, $qcloud);
        $signature = Auth::createReusableSignature($expired, $bucket, $filepath = null, $qcloud);
     ...
     */
    private static function generateResUrl($bucket, $dstPath, $qcloud) {
        $endPoint = "http://region.file.myqcloud.com/files/v2/";
        $endPoint = str_replace('region', $qcloud['region'], $endPoint);

        return $endPoint . $qcloud['appid'] . '/' . $bucket . $dstPath;
    }
