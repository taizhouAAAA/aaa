...
<div class="form-group">
    <label class="col-sm-2 control-label">是否创建直接领取链接</label>
    <div class="col-sm-9 col-xs-12" >
        <?php if( ce('sale.coupon' ,$item) ) { ?>
        <label class="radio-inline">
            <input type="radio" name="quickget" value="0" <?php  if($item['quickget'] == 0) { ?>checked="true"<?php  } ?>  onclick="$('.quickget').hide()"/> 不可以
        </label>
        <label class="radio-inline">
            <input type="radio" name="quickget" value="1" <?php  if($item['quickget'] == 1) { ?>checked="true"<?php  } ?> onclick="$('.quickget').show()" /> 可以
        </label>

        <span class='help-block'>会员是否可以通过链接直接获取优惠券.注意!!!此功能无法与支付与积分兑换优惠券同时使用!!!</span>
