...
<ul class="we7-page-tab">
	...
	<li <?php  if($_GPC['operate'] == 'wechat_refund') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('paySetting', array('direct' => 1, 'operate' => 'wechat_refund'))?>">微信退款设置</a></li>
	<li <?php  if($_GPC['operate'] == 'ali_refund') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('paySetting', array('direct' => 1, 'operate' => 'ali_refund'))?>">支付宝退款设置</a></li>
...
<form action="" method="post" class="we7-form" id="js-pay-setting" ng-controller="storePaySettingCtrl" enctype="multipart/form-data" ng-cloak>
	...
	<div class="form-group">
		<label class="control-label col-sm-2">开关</label>
		<div class="col-sm-8">
			<input id='radio-5' type="radio" name='switch' <?php  if($wechat_refund['switch'] == 1) { ?>checked<?php  } ?> value="1"/>
			<label for="radio-5">开启 </label>
			<input id='radio-6' type="radio" name='switch' <?php  if($wechat_refund['switch'] != 1) { ?>checked<?php  } ?> value="2"/>
	...
	<?php  if($operate == 'ali_refund') { ?>
	<div class="form-group">
		<label class="control-label col-sm-2">支付宝退款</label>
		<div class="col-sm-8">
			<div class="alert alert-warning">
				商户私钥生成教程:&nbsp;<a href="//openclub.alipay.com/read.php?tid=2177&fid=46" class="color-default" target="_blank">去查看</a>
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		<label class="control-label col-sm-2">开关</label>
		<div class="col-sm-8">
			<input id='radio-7' type="radio" name='switch' <?php  if($ali_refund['switch'] == 1) { ?>checked<?php  } ?> value="1"/>
			<label for="radio-7">开启 </label>
			<input id='radio-8' type="radio" name='switch' <?php  if($ali_refund['switch'] != 1) { ?>checked<?php  } ?> value="2"/>
