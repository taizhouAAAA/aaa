...
<div id="js-store-orders" ng-controller="storeOrdersCtrl" ng-cloak>
	...
	<form action="<?php  echo $this->createWebUrl('orders', array('direct' => 1))?>" class="search-box we7-margin-bottom" method="post">
		<select class="we7-margin-right">
			<option <?php  if($_GPC['type'] == '') { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1))?>">全部</option>
			<option <?php  if($_GPC['type'] == STORE_ORDER_PLACE) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'type' => STORE_ORDER_PLACE))?>">待支付</option>
			<option <?php  if($_GPC['type'] == STORE_ORDER_FINISH) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'type' => STORE_ORDER_FINISH))?>">已完成</option>
			<option <?php  if($_GPC['type'] == STORE_ORDER_APPLY_REFUND) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'type' => STORE_ORDER_APPLY_REFUND))?>">申请退款</option>
			<option <?php  if($_GPC['type'] == STORE_ORDER_REFUNDED) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'type' => STORE_ORDER_REFUNDED))?>">已退款</option>
		</select>
		<select class="we7-margin-right">
			<option <?php  if($_GPC['is_wish'] == STORE_ORDER_ALL ) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'is_wish' => STORE_ORDER_ALL))?>">全部订单</option>
			<option <?php  if($_GPC['is_wish'] == STORE_ORDER_NORMAL) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'is_wish' => STORE_ORDER_NORMAL))?>">普通订单</option>
			<option <?php  if($_GPC['is_wish'] == STORE_ORDER_WISH) { ?>selected<?php  } ?> data-url="<?php  echo $this->createWebUrl('orders', array('direct' => 1, 'is_wish' => STORE_ORDER_WISH))?>">预购订单</option>
		</select>
...
<script>
	...
	angular.module('storeApp').value('config', {
		...
		'order_types' : {
			'STORE_ORDER_PLACE': <?php echo STORE_ORDER_PLACE;?>,
			'STORE_ORDER_DELETE': <?php echo STORE_ORDER_DELETE;?>,
			'STORE_ORDER_FINISH': <?php echo STORE_ORDER_FINISH;?>,
			'STORE_ORDER_DEACTIVATE': <?php echo STORE_ORDER_DEACTIVATE;?>,
			'STORE_ORDER_APPLY_REFUND': <?php echo STORE_ORDER_APPLY_REFUND;?>,
			'STORE_ORDER_REFUND_FAIL': <?php echo STORE_ORDER_REFUND_FAIL;?>,
			'STORE_ORDER_REFUNDED': <?php echo STORE_ORDER_REFUNDED;?>,
			'STORE_ORDER_FOUNDER_REFUND': <?php echo STORE_ORDER_FOUNDER_REFUND;?>,

			'STORE_ORDER_WISH': <?php echo STORE_ORDER_WISH;?>,
			'STORE_ORDER_NORMAL': <?php echo STORE_ORDER_NORMAL;?>,
			'STORE_ORDER_ALL': <?php echo STORE_ORDER_ALL;?>,
		},

		'goods_types' : {
			'STORE_TYPE_ACCOUNT': <?php echo STORE_TYPE_ACCOUNT;?>,
			'STORE_TYPE_WXAPP': <?php echo STORE_TYPE_WXAPP;?>,
			'STORE_TYPE_PACKAGE': <?php echo STORE_TYPE_PACKAGE;?>,
			'STORE_TYPE_API': <?php echo STORE_TYPE_API;?>,
			'STORE_TYPE_ACCOUNT_RENEW': <?php echo STORE_TYPE_ACCOUNT_RENEW;?>,
			'STORE_TYPE_WXAPP_RENEW': <?php echo STORE_TYPE_WXAPP_RENEW;?>,
			'STORE_TYPE_USER_PACKAGE': <?php echo STORE_TYPE_USER_PACKAGE;?>,
			'STORE_TYPE_ACCOUNT_PACKAGE': <?php echo STORE_TYPE_ACCOUNT_PACKAGE;?>,

			'STORE_TYPE_MODULE': <?php echo STORE_TYPE_MODULE;?>,
			'STORE_TYPE_WXAPP_MODULE': <?php echo STORE_TYPE_WXAPP_MODULE;?>,
			'STORE_TYPE_WEBAPP_MODULE': <?php echo STORE_TYPE_WEBAPP_MODULE;?>,
			'STORE_TYPE_PHONEAPP_MODULE': <?php echo STORE_TYPE_PHONEAPP_MODULE;?>,
			'STORE_TYPE_XZAPP_MODULE': <?php echo STORE_TYPE_XZAPP_MODULE;?>,
			'STORE_TYPE_ALIAPP_MODULE': <?php echo STORE_TYPE_ALIAPP_MODULE;?>,
			'STORE_TYPE_BAIDUAPP_MODULE': <?php echo STORE_TYPE_BAIDUAPP_MODULE;?>,
			'STORE_TYPE_TOUTIAOAPP_MODULE': <?php echo STORE_TYPE_TOUTIAOAPP_MODULE;?>,
		},
