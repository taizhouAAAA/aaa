	...
	<table class="table we7-table vertical-middle">
		...
		<tr>
			...
			<td class="text-left">
				...
				<?php  } else if(in_array($goods['type'], array_keys($store_goods_types['account_num']))) { ?>
				创建<?php  echo $goods['platform_num'];?>个<?php  echo $store_goods_types['account_num'][$goods['type']]['title'];?>
				<?php  } else if(in_array($goods['type'], array_keys($store_goods_types['renew']))) { ?>
			...
			<td>
				...
				<?php  if(!in_array($goods['type'], array_keys($store_goods_types['account_num']))) { ?>
					...
					<?php  if($goods['unit'] == 'month') { ?>
						<?php  if(is_array($renews)) { foreach($renews as $goods_type => $renew) { ?>
							<?php  if($goods['type'] == $goods_type) { ?>
								<?php  echo $goods[$renew['num_sign']];?>
							<?php  } ?>
						<?php  } } ?>月
			...
			<td ng-if="!status">
				<div class="link-group">
					<?php  if(in_array($goods['type'], array_keys($renews))) { ?>
					<a href="<?php  echo $this->createWebUrl('goodspost', array('id' => $goods['id'], 'type' => $goods['type'], 'direct' => 1))?>">编辑</a>
					<?php  } else { ?>
					<a href="<?php  echo $this->createWebUrl('goodspost', array('id' => $goods['id'], 'type' => $goods['type'], 'direct' => 1))?>">编辑</a>
					<?php  } ?>
